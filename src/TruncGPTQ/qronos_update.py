...
def qronos_single_layer_update(
    weight: torch.Tensor,
    ...
    """
    Linear-only Qronos-style update.
    `quant_fn` must return dequantized weights with the same shape/device as input.
    """
    if weight.ndim != 2:
    ...
    # Step 1: set first column using Gw - Uv
    Gw0 = W_orig_p.matmul(Gp[:, 0] * Dhi[0])
    Uv0 = Wp.matmul(Uh[0, :] * Dhi[0])
    Wp[:, 0] = Gw0 - Uv0

    # Sherman-Morrison-Woodbury reduction for tail [1:]
    if iH.shape[0] > 1:
        c = iH[0, 0]
        b = iH[1:, [0]]
    ...
    if iH_tail.numel() > 0:
        ...
        # Step 2+: blockwise correction on remaining coordinates.
    ...
    # Final quantization pass on updated float weights
